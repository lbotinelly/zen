...
namespace Zen.Web
{
    public class ZenConfigurationLoader
    {
        private bool _loaded = false;

        internal ZenConfigurationLoader(Setup.ZenServerOptions options, IConfiguration configuration)
        {
        ...
        public Setup.ZenServerOptions Options { get; }
        ...
        /// </summary>
        public ZenConfigurationLoader Endpoint(string name, Action<EndpointConfiguration> configureOptions)
        {
        ...
        /// </summary>
        public ZenConfigurationLoader Endpoint(IPAddress address, int port) => Endpoint(address, port, _ => { });

        /// <summary>
        /// Bind to given IP address and port.
        /// </summary>
        public ZenConfigurationLoader Endpoint(IPAddress address, int port, Action<ListenOptions> configure)
        {
            if (address == null)
            {
        ...
        /// </summary>
        public ZenConfigurationLoader Endpoint(IPEndPoint endPoint) => Endpoint(endPoint, _ => { });

        /// <summary>
        /// Bind to given IP address and port.
        /// </summary>
        public ZenConfigurationLoader Endpoint(IPEndPoint endPoint, Action<ListenOptions> configure)
        {
            if (endPoint == null)
            {
        ...
        /// </summary>
        public ZenConfigurationLoader LocalhostEndpoint(int port) => LocalhostEndpoint(port, options => { });

        /// <summary>
        /// Listens on ::1 and 127.0.0.1 with the given port. Requesting a dynamic port by specifying 0 is not supported
        /// for this type of endpoint.
        /// </summary>
        public ZenConfigurationLoader LocalhostEndpoint(int port, Action<ListenOptions> configure)
        {
            if (configure == null)
            {
                throw new ArgumentNullException(nameof(configure));
        ...
        /// </summary>
        public ZenConfigurationLoader AnyIPEndpoint(int port) => AnyIPEndpoint(port, options => { });

        /// <summary>
        /// Listens on all IPs using IPv6 [::], or IPv4 0.0.0.0 if IPv6 is not supported.
        /// </summary>
        public ZenConfigurationLoader AnyIPEndpoint(int port, Action<ListenOptions> configure)
        {
            if (configure == null)
            {
                throw new ArgumentNullException(nameof(configure));
        ...
        /// </summary>
        public ZenConfigurationLoader UnixSocketEndpoint(string socketPath) => UnixSocketEndpoint(socketPath, _ => { });

        /// <summary>
        /// Bind to given Unix domain socket path.
        /// </summary>
        public ZenConfigurationLoader UnixSocketEndpoint(string socketPath, Action<ListenOptions> configure)
        {
            if (socketPath == null)
            {
        ...
        /// </summary>
        public ZenConfigurationLoader HandleEndpoint(ulong handle) => HandleEndpoint(handle, _ => { });

        /// <summary>
        /// Open a socket file descriptor.
        /// </summary>
        public ZenConfigurationLoader HandleEndpoint(ulong handle, Action<ListenOptions> configure)
        {
            if (configure == null)
            {
                throw new ArgumentNullException(nameof(configure));
        ...
        private void LoadDefaultCert(ConfigurationReader configReader)
        {
            ...
            //else
            //{
            //    var logger = Options.ApplicationServices.GetRequiredService<ILogger<ZenServer>>();
        ...
        private X509Certificate2 FindDeveloperCertificateFile(ConfigurationReader configReader, ILogger<ZenServer> logger)
        {
